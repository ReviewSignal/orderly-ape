...
// MustAlphaNumericString returns a cryptographically secure random sequence of
// alphanumeric characters and panics on error.
func MustAlphaNumericString(length int) string {
	if length < 1 {
		panic("length must be greater than 0")
	}
	s, err := AlphaNumericString(length)
	if err != nil {
		panic(err)
	}
	return s
...
// MustLowerAlphaNumericString returns a cryptographically secure random sequence of
// lower alphanumeric characters and panics on error.
func MustLowerAlphaNumericString(length int) string {
	if length < 1 {
		panic("length must be greater than 0")
	}
	s, err := LowerAlphaNumericString(length)
	if err != nil {
		panic(err)
	}
	return s
...
// MustASCIIString returns a cryptographically secure random sequence of
// printable ASCII characters, excluding space and panics on error.
func MustASCIIString(length int) string {
	if length < 1 {
		panic("length must be greater than 0")
	}
	s, err := ASCIIString(length)
	if err != nil {
		panic(err)
	}
	return s
